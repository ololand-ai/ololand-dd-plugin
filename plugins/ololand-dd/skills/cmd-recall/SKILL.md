---
name: ololand-dd-recall
description: "Use when the user asks to run the OloLand /recall workflow. Read confirmed facts about an OloLand deal from deal_session_memory. Use at the start of any session about a deal to load the analyst-confirmed context — EBITDA, sponsor, IC date, conventions — without re-asking the user."
---

<!-- Generated by scripts/generate-plugin-artifacts.py; do not edit. -->

# Codex wrapper for /recall

Follow the OloLand command instructions below when the user asks for `/recall` or the equivalent workflow in Codex.
