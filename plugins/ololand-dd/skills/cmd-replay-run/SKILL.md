---
name: ololand-dd-replay-run
description: "Use when the user asks to run the OloLand /replay-run workflow. Re-run an existing OloLand agent run against the current harness + skills (or byte-for-byte against the original methodology bytes). Admin / compliance role required. The regulator-grade reproducibility surface — used to demonstrate that a graded output can be reconstructed from snapshots, and to A/B the current prompt against last week's data."
---

<!-- Generated by scripts/generate-plugin-artifacts.py; do not edit. -->

# Codex wrapper for /replay-run

Follow the OloLand command instructions below when the user asks for `/replay-run` or the equivalent workflow in Codex.
