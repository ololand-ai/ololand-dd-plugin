---
name: ololand-dd-verify
description: "Use when the user asks to run the OloLand /verify workflow. Verify every number and citation in an OloLand artifact (CIM paragraph, IC memo, risk narrative, DCF write-up) against the deal's source documents and deterministic engine runs. Runs the four atomic-claim verifiers + optional citation-marker audit + cross-document reconciliation. The 'defend every number, cite every source' button."
---
